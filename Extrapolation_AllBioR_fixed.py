...
# -----------------------------------------------------------------------------
# Build the complete trajectories ONCE.
# The order is [Glucose, DO, pH, Biomass] for each bioreactor.
# -----------------------------------------------------------------------------
ts_list_full = []
ys_list_full = []
...
for o in range(1, len(glucose_df.columns)):
    # --- GLUCOSE ---
    s = glucose_df[glucose_df.columns[o]].dropna()
    ts = glucose_df.loc[s.index, "Time"]
    ts_list_full.append(ts.to_numpy(dtype=np.float32))
    ys_list_full.append(s.to_numpy(dtype=np.float32))

    # --- DO, pH, Biomass ---
    cols = df_merged.columns[1:][3*o-3:3*o]
    s1 = df_merged[cols[0]].dropna()
    s2 = df_merged[cols[1]].dropna()
    s3 = df_merged[cols[2]].dropna()
    ...
    ts_list_full.extend([
        ts1.to_numpy(dtype=np.float32),
        ts2.to_numpy(dtype=np.float32),
        ts3.to_numpy(dtype=np.float32),
    ])
    ys_list_full.extend([
        s1.to_numpy(dtype=np.float32),
        s2.to_numpy(dtype=np.float32),
        s3.to_numpy(dtype=np.float32),
    ])

list_bior = [j[11:13] for j in glucose_df.columns[1:]]
list_values = []

# Leave-one-bioreactor-out evaluation.
for test_bior in range(len(list_bior)):
    start = 4 * test_bior
    stop = start + 4

    # -------------------------------------------------------------------------
    # Train ONCE for this held-out bioreactor, using the COMPLETE trajectories
    # of all other bioreactors.
    # -------------------------------------------------------------------------
    train_raw = ys_list_full[:start] + ys_list_full[stop:]
    train_t = ts_list_full[:start] + ts_list_full[stop:]
    test_raw = ys_list_full[start:stop]
    ...
    test = normalize(test_raw, stats)

    n_epochs = 50000  # Max is 240000
    loss, R, t, ys, model = main(train, train_t, n_epochs)

    # Training diagnostics for this held-out bioreactor.
    ...
    # -------------------------------------------------------------------------
    # Build the COMPLETE normalized test trajectory once.
    # -------------------------------------------------------------------------
    glc_ys, od_ys, ph_ys, bio_ys = test
    glc_ts, od_ts, ph_ts, bio_ts = test_t
    ...
    # Reset results for THIS bioreactor only.
    # One block of 16 values is added per cutoff, in order 80%, 70%, ..., 10%.
    extrap_values = []
    ...
    for i in range(8, 0, -1):
        ...
        y_true_extrap = torch.tensor(
            np.asarray(y_true_extrap_jax), dtype=torch.float32
        )
        y_pred_extrap = torch.tensor(
            np.asarray(y_pred_extrap_jax), dtype=torch.float32
        )
        ...
        for var_idx, name in enumerate(var_names):
            ...
            axs[var_idx].plot(
                t_test, ys_test[:, var_idx], label="Ground truth", linewidth=2
            )
            axs[var_idx].plot(
                t_test[cut_off:],
        ...
        plt.tight_layout()
        fig_name = (
            '/shared/projects/pinn_proj/scripts/extrapolation/results_'
            + str(list_bior[test_bior]) + '_' + str(i)
        )
        plt.savefig(fig_name + '.png', format='png')
        plt.close(fig)

        # Store metrics in the same order for every cutoff:
    ...
    list_values.append(extrap_values)
    # -------------------------------------------------------------------------
    # Plot the 8 cutoff results for THIS bioreactor.
    # The order matches the insertion order above: 80%, ..., 10%.
    # -------------------------------------------------------------------------
    ...
    for metric_idx, metric_name in enumerate(val_names):
        ...
        fig.suptitle(
            f"Bioreactor {list_bior[test_bior]} - Forecast {metric_name} vs observed fraction",
            fontsize=16,
        )

        for var_idx, (var_name, ax) in enumerate(zip(var_names, axs.flatten())):
            values = [
                extrap_values[block * 16 + var_idx * 4 + metric_idx]
        ...
        fig_name = (
            '/shared/projects/pinn_proj/scripts/extrapolation/results_'
            + str(list_bior[test_bior]) + '_' + str(metric_name)
        )
        plt.savefig(fig_name + '.png', format='png')
        plt.close(fig)

# Convert results to array:
# shape = (n_bioreactors, 128)
# 128 = 8 cutoffs * 4 variables * 4 metrics
all_results = np.asarray(list_values)

# Fractions are in the SAME order as they were computed:
# block 0 = 80%, block 1 = 70%, ..., block 7 = 10%
x = np.arange(0.8, 0, -0.1)
...
for metric_idx, metric_name in enumerate(val_names):
    ...
    fig.suptitle(
        f"Distribution du score {metric_name} selon la fraction observée",
        fontsize=16
    )

    for var_idx, (var_name, ax) in enumerate(
        zip(var_names, axs.flatten())
    ):
        ...
        for block in range(8):
            ...
            # One score per bioreactor for this cutoff
            scores = all_results[:, column_idx]
        ...
        ax.set_title(var_name)
        ax.set_xlabel("Fraction de trajectoire observée")
        ax.set_ylabel(metric_name)

        ax.set_xticks(x)
        ax.set_xticklabels(
            [f"{fraction:.1f}" for fraction in x]
        )
    ...
    fig_name = (
        '/shared/projects/pinn_proj/scripts/extrapolation/'
        'final_results_' + str(metric_name)
    )
