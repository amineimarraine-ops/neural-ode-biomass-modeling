...
for prct in range(8,0,-1):
    ...
    plt.tight_layout()
    fig_name = '/shared/projects/pinn_proj/scripts/extrapolation/losses_bioreactor' + list_bior[test_bior]
    ...
    # Find the cutoff time — where training data ended
    t_cutoff = t_test[int(prct * len(t_test) // 10)]
    extrap_mask = np.asarray(t_test) >= float(t_cutoff)

    y_true_extrap = y_true_test[extrap_mask]
    y_pred_extrap = y_pred_test_torch[extrap_mask]
    ...
    for var_idx, name in enumerate(var_names):
        ...
        # R2 on extrapolated part only
        ss_res_e = torch.sum((y_true_extrap[:, var_idx] - y_pred_extrap[:, var_idx]) ** 2)
        ss_tot_e = torch.sum((y_true_extrap[:, var_idx] - torch.mean(y_true_extrap[:, var_idx])) ** 2)
        r2_extrap = 1 - ss_res_e / (ss_tot_e + 1e-12)
        ...
        axs[var_idx].axvline(x=float(t_cutoff), color='red', linestyle=':', linewidth=1.5, label="Extrap. start")
        ...
        axs[var_idx].text(t_test[-1] * 0.6, ymin + 0.95*(ymax-ymin), f"R² full = {r2_full:.4f}", fontsize=9)
        axs[var_idx].text(t_test[-1] * 0.6, ymin + 0.85*(ymax-ymin), f"R² extrap = {r2_extrap:.2f}", fontsize=9, color='red')
        axs[var_idx].grid(True)

        plt.tight_layout()
        fig_name = '/shared/projects/pinn_proj/scripts/extrapolation/curvs_bioreactor_' + prct
        plt.savefig(fig_name+'.png', format='png') ###########################################
        
    var_names = ["Glucose", "OD", "pH", "Biomass"]
    for var_idx, name in enumerate(var_names):
        ss_res = torch.sum((y_true_extrap[:, var_idx] - y_pred_extrap[:, var_idx]) ** 2)
        ss_tot = torch.sum((y_true_extrap[:, var_idx] - torch.mean(y_true_extrap[:, var_idx])) ** 2)
        r2 = 1 - ss_res / (ss_tot + 1e-12)
        mae = torch.mean(torch.abs(y_true_extrap[:, var_idx] - y_pred_extrap[:, var_idx]))
        rmse = torch.sqrt(torch.mean((y_true_extrap[:, var_idx] - y_pred_extrap[:, var_idx]) ** 2))
        nrmse = rmse / (torch.max(y_true_extrap[:, var_idx]) - torch.min(y_pred_test_torch[:, var_idx]) + 1e-12)
...
for metric_idx, metric_name in enumerate(val_names):
    ...
    plt.tight_layout()
    fig_name = '/shared/projects/pinn_proj/scripts/extrapolation/results_' + metric_name
    plt.savefig(fig_name+'.png', format='png') ###########################################